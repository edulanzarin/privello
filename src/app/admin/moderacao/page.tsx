...
export default async function AdminModeracaoPage({ searchParams }: PageProps) {
  ...
  const [queueRows, queueTotal] = await Promise.all([
    prisma.verificationCase.findMany({
      where: queueWhere,
      orderBy: sortFilter === "newest"? { createdAt: "desc"}
        : { waitingSince: "asc"},
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <form method="get"action="/admin/moderacao"className="flex items-center gap-2">
                  ...
                  <Input
                    name="q"defaultValue={searchQ}
                    placeholder="Buscar por nome…"className="w-40 px-2.5 py-1.5 text-xs"/>
                    ...
                    <TD>
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Link
                          ...
                          target="_blank"className="rounded border border-line px-2 py-1 text-2xs font-bold uppercase text-ink-dim hover:border-ink/30 hover:text-ink transition">
                          Perfil ↗
                        </Link>
                      </div>
                    </TD>
