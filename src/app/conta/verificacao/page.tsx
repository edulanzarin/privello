...
function Sidebar({ step }: { step: number }) {
  return (
    <aside className="flex w-full flex-col bg-sidebar px-8 py-12 text-white md:max-w-[280px] md:min-h-screen md:sticky md:top-0 md:h-screen md:overflow-auto">
      <div>
      ...
      <div className="mt-8">
        ...
        <div className="h-1 w-full rounded-full bg-white/[0.08] overflow-hidden">
          <div className="h-full rounded-full bg-coral transition-all duration-500 ease-out" style={{ width: `${(step / STEPS.length) * 100}%` }} />
        </div>
      </div>
...
export default function ContaVerificacaoPage() {
  ...
  const stepDone = (s: typeof STEPS[0]) => s.fields.every((f) => s.optional ? true : !!slots[f.key]?.url);
      ...
      <main className="flex-1 bg-background px-5 py-10 md:px-14 md:py-14">
        ...
        <div className="mb-8">
          <div className="flex items-start gap-4 mb-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-foreground/[0.06] text-foreground">
        ...
        {/* Upload slots */}
        <div className={cn("grid gap-5", currentStep.fields.length > 1 ? "md:grid-cols-2" : "max-w-md")}>
                ...
                <div className="p-5">
                  ...
                  {done ? (
                    ...
                    <>
                      ...
                      <input ref={(el) => { inputRefs.current[field.key] = el; }} type="file" accept={field.accept} className="sr-only" onChange={(e) => { if (e.target.files?.[0]) uploadFile(field.key, e.target.files[0]); }} />
                    </>
                  )}
                </div>
              </div>
