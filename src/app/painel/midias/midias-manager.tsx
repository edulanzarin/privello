...
"flex items-center gap-1.5 px-4 py-3 text-sm font-semibold transition",
                visTab === t.key
                  ? "border-b-2 border-rose text-ink -mb-px": "text-ink-dim hover:text-ink",
              )}>
              {t.label}
              <span className={cn(
...
"flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium transition",
                  typeTab === t.key
                    ? "border-b-2 border-ink text-ink -mb-px": "text-ink-dim hover:text-ink",
                )}>
        ...
        {filtered.length === 0 ? (
          <EmptyState
            title="Nenhum item aqui ainda"description="Adicione fotos ou vídeos no painel à direita."icon={<ImagePlus className="h-10 w-10"strokeWidth={1} />}
          />
        ) : (
          <>
            <div className="grid grid-cols-3 gap-2">
              {shown.map((m) => {
                ...
                return (
                  <button key={m.id} type="button"className={cn(
"group relative overflow-hidden rounded-xl",
                  ...
                  >
                    {isVideo(m) ? (
                      <video src={m.url}
                        className="h-full w-full object-cover transition duration-300 group-hover:brightness-75"muted playsInline />
                    ) : (
                      <Image src={m.url} alt=""fill
                        className="object-cover transition duration-300 group-hover:brightness-75"sizes="33vw"/>
                    )}
                    {isVideo(m) && (
      ...
      <div className="space-y-5 rounded-2xl border border-line bg-white p-5 shadow-sm lg:sticky lg:top-24 lg:self-start">
        ...
        {previews.length > 0 ? (
          <div className="grid grid-cols-3 gap-1.5">
            ...
            <button type="button"onClick={() => fileRef.current?.click()}
              className="flex items-center justify-center border-2 border-dashed border-black/[0.12] rounded-xl bg-white text-ink-dim hover:border-rose hover:text-rose transition"style={{ aspectRatio: "1/1"}}>
        ...
        <div>
          ...
          <input
            ...
            maxLength={150}
            placeholder="Uma breve descrição…"className="w-full rounded-lg border border-line px-3 py-[7px] text-md shadow-[inset_0_0.5px_2px_rgba(0,0,0,0.04)] outline-none focus-visible:ring-2 focus-visible:ring-rose/40 focus-visible:ring-offset-2 focus-visible:ring-offset-background transition-all hover:border-black/20 focus:border-rose focus:shadow-[0_0_0_3px_rgba(10,132,255,0.25)]"/>
        ...
        {curItem && (
          <>
            ...
            <div className="flex h-14 shrink-0 items-center justify-between px-4">
              <button
                onClick={closeLb}
                className="inline-flex min-h-[44px] min-w-[44px] items-center gap-2 text-white/70 transition hover:text-white"aria-label="Voltar">
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex shrink-0 flex-col gap-2">
                    {visTab === "publica"&& !curItem.isCover && (
                      <button
                        type="button"onClick={() => handleSetCover(curItem.id)}
                    ...
                    {curItem.isCover ? (
                      <p className="text-2xs text-white/30 text-center max-w-[100px]">
                        Defina outra foto de perfil antes de remover
                      </p>
                    ) : (
                      <button
                        type="button"onClick={() => handleRemove(curItem.id)}
