...
- [ ] 1. Validar pré-condições (Spawn-Readiness Gate)
  - [ ] 1.1 Confirmar `requirements.md > §4` registrada como `n/a — fase não toca APIs do Next.js`
  ...
  - [ ] 1.2 Confirmar fase-4 e fase-5 em `Done` no master
  ...
  - [ ] 1.3 Confirmar §3 (`OutOfScopeFinding`) com as 2 linhas herdadas e demais inicialmente vazias
...
- [ ] 2. Inventário baseline
  - [ ] 2.1 Confirmar 11 mockups em `design/`
  ...
  - [ ] 2.2 Capturar projects atuais em `playwright.config.ts`
  ...
  - [ ] 2.3 Inventariar `Critical_Control`s sem 44×44 px declarados
  ...
  - [ ] 2.4 Inventariar Modais consumidores e classificar em 4 categorias
  ...
  - [ ] 2.5 Confirmar gestos atuais (baseline)
...
- [ ] 3. Wave Browser Matrix
  - [ ] 3.1 Documentar a matriz canônica em `mockups-diff.md > §Browser Matrix`
  ...
  - [ ] 3.2 Estender `playwright.config.ts > projects` para 4 projects rodáveis
  ...
  - [ ] 3.3 Validar `npm run test:e2e -- --list` lista specs em 4 projects
  ...
  - [ ] 3.4 Documentar smoke browser manual para desktop Safari + desktop Edge
...
- [ ] 4. Wave Touch Target 44×44
  - [ ] 4.1 Aplicar `min-h-[44px] min-w-[44px]` em controles da categoria (a) — botões de ação primários
  ...
  - [ ] 4.2 Aplicar 44×44 em controles da categoria (b) — ícones de navegação
  ...
  - [ ] 4.3 Aplicar 44×44 em controles da categoria (c) — botão fechar Modal
  ...
  - [ ] 4.4 Aplicar 44×44 em controles da categoria (d) — like/favorite/comments em mídia
  ...
  - [ ] 4.5 Validar 44×44 em controles das categorias (e), (f), (g) — switches, dropdown triggers, chevrons
  ...
  - [ ] 4.6 Re-rodar inventário e confirmar count
...
- [ ] 5. Wave Teclado Virtual
  - [ ] 5.1 Adicionar `interactiveWidget: "resizes-content"` em `<meta name="viewport">`
  ...
  - [ ] 5.2 Confirmar `100dvh` em `reels-feed.tsx` preservado
  ...
  - [ ] 5.3 Smoke browser manual no fluxo de login
  ...
  - [ ] 5.4 Smoke browser manual no fluxo de cadastro
  ...
  - [ ] 5.5 Smoke browser manual no fluxo de comentário
  ...
  - [ ] 5.6 Smoke browser manual no fluxo de suporte
  ...
  - [ ] 5.7 Decidir criação opcional de `useVirtualKeyboard`
...
- [ ] 6. Wave Bottom-sheet
  - [ ] 6.1 Decidir criação opcional de `useMediaQuery`
  ...
  - [ ] 6.2 Aplicar bottom-sheet em `src/app/conta/perfil/client-profile-edit.tsx`
  ...
  - [ ] 6.3 Aplicar bottom-sheet nos demais sites classificados como `bottom_sheet_em_mobile`
  ...
  - [ ] 6.4 Preservar Modal primitivo intocado
    - Confirmar via `git diff src/components/ui/modal.tsx` — zero alterações
    - Se houver alteração, é regressão: reverter
    - _Requirements: 4.6_

- [ ] 7. Wave Diff Visual com 11 mockups
  - [ ] 7.1 Criar `mockups-diff.md` em `c:\Users\edulanzarin\Documents\Dev\privello\.kiro\specs\fase-6-mobile-cross-browser\`
  ...
  - [ ] 7.2 Diff visual: Mockup 1 — `Dashboard _ vis_o geral.png` × `src/app/painel/page.tsx`
  ...
  - [ ] 7.3 Diff visual: Mockup 2 — `Fila de modera_o.png` × `src/app/admin/moderacao/page.tsx`
    - Mesma estrutura
    - _Requirements: 5.2, 5.3_

  - [ ] 7.4 Diff visual: Mockup 3 — `Financeiro _Premium_.png` × `src/app/painel/financeiro/page.tsx`
    - Mesma estrutura
    - _Requirements: 5.2, 5.3_

  - [ ] 7.5 Diff visual: Mockup 4 — `Home _ landing.png` × `src/app/page.tsx`
    - Mesma estrutura
    - _Requirements: 5.2, 5.3_

  - [ ] 7.6 Diff visual: Mockup 5 — `Listagem com filtros.png` × `src/app/buscar/page.tsx` ou `src/app/descobrir/[citySlug]/page.tsx`
    - Decidir tela alvo correta caso a caso (provável: `/buscar`)
    - _Requirements: 5.2, 5.3_

  - [ ] 7.7 Diff visual: Mockup 6 — `Onboarding _ Passo 03 _ Fotos.png` × `src/app/conta/onboarding/fotos/page.tsx`
    - Mesma estrutura
    - _Requirements: 5.2, 5.3_

  - [ ] 7.8 Diff visual: Mockup 7 — `Perfil p_blico.png` × `src/app/p/[slug]/page.tsx`
    - Mesma estrutura
    - _Requirements: 5.2, 5.3_

  - [ ] 7.9 Diff visual: Mockup 8 — `Planos _ pre_os.png` × `src/app/planos/page.tsx`
    - Mesma estrutura
    - _Requirements: 5.2, 5.3_

  - [ ] 7.10 Diff visual: Mockup 9 — `Solicita_es pendentes.png` × `src/app/painel/page.tsx` (cards de solicitação)
    - Confirmar se é `/painel` ou rota dedicada existente; se for feature não-implementada, marcar `tela_alvo: nao_implementada`
    - _Requirements: 5.2, 5.3, 5.6_

  - [ ] 7.11 Diff visual: Mockup 10 — `Solicitar encontro _cliente logado_.png` × `src/app/solicitar/[slug]/page.tsx`
    - Mesma estrutura
    - _Requirements: 5.2, 5.3_

  - [ ] 7.12 Diff visual: Mockup 11 — `Verifica_o de identidade.png` × `src/app/conta/verificacao/page.tsx`
    - Mesma estrutura
    - _Requirements: 5.2, 5.3_

  - [ ] 7.13 Aplicar correções de divergências `a corrigir` que caibam no escopo das demais Waves
...
- [ ] 8. Wave Gestos
  - [ ] 8.1 Aplicar `touch-action: none` (Tailwind: `touch-none`) em lightboxes/story viewers fullscreen
    - Sites alvo:
  ...
  - [ ] 8.2 Aplicar `overscroll-behavior: contain` (Tailwind: `overscroll-contain`/`overscroll-y-contain`) em containers com scroll interno
    - Sites alvo:
  ...
  - [ ] 8.3 Aplicar `overscroll-behavior-y: none` em rotas onde pull-to-refresh é destrutivo
  ...
  - [ ] 8.4 Documentar a tabela de gestos em `mockups-diff.md > §Gestos`
  ...
  - [ ] 8.5 Smoke browser manual em iOS Safari + Android Chrome
...
- [ ] 9. Wave `<MediaLightbox>` responsivo
  - [ ] 9.1 Decidir local do primitivo
  ...
  - [ ] 9.2 Criar `<MediaLightbox>` como wrapper sobre Modal
  ...
  - [ ] 9.3 Substituir overlay inline em `src/components/profile/media-gallery.tsx:178` por `<MediaLightbox>`
  ...
  - [ ] 9.4 Smoke test determinístico para `<MediaLightbox>`
...
- [ ] 10. Wave `<Drawer>` mobile
  - [ ] 10.1 Decidir criação do primitivo `<Drawer>`
...
- [ ] 11. Wave Bottom nav redesign
  - [ ] 11.1 Aplicar redesign pontual em `src/components/layout/bottom-nav.tsx`
  ...
  - [ ] 11.2 Documentar divergências aceitas / a corrigir em `mockups-diff.md > §Bottom nav redesign`
    - Cross-reference às entradas dos mockups no §Mockups quando aplicável
    - _Requirements: 7.5_

- [ ] 12. PBTs co-localizados (gate de testes)
  - [ ] 12.1 * Implementar Property 1 — bottom-sheet responsivo determinístico
  ...
  - [ ] 12.2 * Implementar Property 2 — Critical_Control bounding rect / className
  ...
  - [ ] 12.3 Validar suite completa de testes não regrediu
...
- [ ] 13. Smoke checks finais
  - [ ] 13.1 `npm run lint` — registrar baseline
  ...
  - [ ] 13.5 `npm run test:e2e -- --list` — 4 projects listados
  ...
  - [ ] 13.6 Smoke browser manual final
